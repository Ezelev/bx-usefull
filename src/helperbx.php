...
class HelperBX
{
    ...
    public static function getELementsByDate()
    {
        ...
        $elementsResult = CUser::GetList(($by = "ID") , ($order = "ASC") , $filter, array(
            "SELECT" => array(
                "UF_PASS_REAL",
                "UF_PASS_CHANGE_DATE"
            )
    ...
    /**
     *
     * Возвращает элемен по символьному коду
     *
     * @param $sIBlockCode
     * @param bool $bRefreshCache
     * @return int
     */
    function getElementByCode($elCode, $bRefreshCache = false)
    {
        pr($elCode);
        $obCache = new CPHPCache;
        $iReturnId = 0;
        $CACHE_ID = 'getIBlockIdByCode' . $elCode . '_______';
        ...
        if ($obCache->StartDataCache($iCacheTime, $CACHE_ID)):
            ...
            if (CModule::IncludeModule('iblock'))
            {
                ...
                $dbItems = CIBlockElement::GetList(array(
                    'ID' => 'ASC'
                ) , $arFilter, false);
